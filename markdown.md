...
| **Key pair** | **Proceed without a key pair** (not needed — we use EC2 Instance Connect) |
| **Network settings** | |
| - VPC | Default VPC (or create a new one) |
| - Subnet | Select a public subnet |
| - Auto-assign public IP | **Enable** |
| - Security group | Create new: `CompanyY-SG` |
| -- Rule 1 | HTTP (port 80) from 0.0.0.0/0 |
| -- Rule 2 | HTTPS (port 443) from 0.0.0.0/0 |
| **Storage** | |
...
## Step 3: Connect to the Instance via EC2 Instance Connect

**No SSH key or local terminal needed — this runs entirely in the browser.**

1. Go to **EC2 > Instances** and select your `CompanyY-WebServer` instance.
2. Click the **Connect** button at the top.
3. Select the **EC2 Instance Connect** tab.
4. Leave the username as `ec2-user`.
5. Click **Connect**.

A browser-based terminal session will open, giving you a shell on the instance.

> **If EC2 Instance Connect fails**, try the **Session Manager** tab instead:
> - This requires the SSM Agent to be running (it is by default on Amazon Linux 2023).
> - The instance also needs an IAM role with the `AmazonSSMManagedInstanceCore` policy. See the IAM role setup in Step 5.1 — you may want to do that first, then come back here.
...
- Output of `systemctl status amazon-ssm-agent` (showing active/running)
...
**Note:** The EC2 instance needs an IAM role for both CloudWatch and Session Manager access. If the lab doesn't have this pre-configured:
...
4. Name it `CompanyY-EC2-Role`
5. Go to **EC2 > Instances > CompanyY-WebServer > Actions > Security > Modify IAM role**
6. Select the new role and save

**Do this early** — ideally right after the instance launches (Step 1), so that Session Manager and CloudWatch both work when you need them.
...
| A4b | `dnf repolist` output + `systemctl status` for httpd, chronyd, amazon-ssm-agent, with timestamp | A.4 |
